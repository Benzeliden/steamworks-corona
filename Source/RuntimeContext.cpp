...
LuaEventDispatcher* RuntimeContext::GetLuaEventDispatcher() const
{
	return fLuaEventDispatcherPointer.get();
}
...
void RuntimeContext::OnHandleGlobalSteamEvent(TSteamResultType* eventDataPointer)
{
	...
	// Queue the received Steam event data.
	// Will be dispatched to Lua later once this runtime context verifies that Corona is not suspended.
	if (eventDataPointer)
	{
